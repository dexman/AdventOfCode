...
# Error: That command requires priority 50. You currently have priority 0.  You
# must deposit 50 stars to increase your priority to the required level.  The
# console flickers for a moment, and then prints another message:
...
def parse_blueprint(blueprint_input):
    def parse_rule(rule_lines):
        write = int(rule_lines[0][-2])
        direction = -1 if rule_lines[1].endswith('left.') else 1
        next_state = rule_lines[2][-2]
        return Rule(write, direction, next_state)
        
    header_chunk, *state_chunks = map(
        lambda l: l.splitlines(),
        blueprint_input.split('\n\n'))

    start_state = header_chunk[0][-2]
    step_count = int(re.findall('\d+', header_chunk[1])[0])

    states = {}
    for state_lines in state_chunks:
        name = state_lines[0][-2]
        rules = [
            parse_rule(state_lines[2:5]),
            parse_rule(state_lines[6:9]),
        ]
        states[name] = rules

    return Blueprint(start_state, step_count, states)

def execute(blueprint):
    tape = collections.defaultdict(lambda: 0)
    cursor = 0
    state = blueprint.states[blueprint.start_state]
    for _ in range(blueprint.step_count):
        rule = state[tape[cursor]]
        tape[cursor] = rule.write
        cursor += rule.direction
        state = blueprint.states[rule.next_state]
    return (tape, cursor)

def checksum(tape):
    return sum(filter(lambda x: x == 1, tape.values()))

with open('aoc25.txt', 'r') as f:
    blueprint = parse_blueprint(f.read())
tape, _ = execute(blueprint)
c = checksum(tape)
print(c)
