...
def find_scanner_position(layer_index, picosecond, layers):
    if layer_index not in layers:
        return None
    if picosecond < 0:
        return 0
    layer_max_position = layers[layer_index] - 1
    position = picosecond % layer_max_position
    if (picosecond // layer_max_position) % 2 != 0:
        position = layer_max_position - position
    return position

def level_severity(layer_index, layers):
    if layer_index not in layers:
        return 0
    return layers[layer_index] * layer_index

def trip_severity(layers, delay=0):
    severity = 0
    ever_caught = False
    end_time = delay + max(layers.keys()) + 1
    for picosecond in range(delay, end_time):
        layer_index = picosecond - delay
        scanner_position = find_scanner_position(layer_index, picosecond, layers)
        caught = scanner_position == 0
        if caught:
            severity += level_severity(layer_index, layers)
            ever_caught = True
    return severity, ever_caught

def parse_levels(levels_input):
    def parse_level(line):
        level, range = map(int, line.split(': '))
        return (level, range)
    return dict(map(parse_level, levels_input.strip().splitlines()))

with open('aoc13.txt', 'r') as f:
    layers = parse_levels(f.read())

severity, _ = trip_severity(layers)
print(f'Part 1: Severity is {severity}.')

safe_delay = None
for delay in range(4000000):
    _, caught = trip_severity(layers, delay=delay)
    if not caught:
        safe_delay = delay
        break
    if delay % 10000 == 0:
        print(f'Delay {delay}...')
print(f'Part 2: Minimum safe delay is {safe_delay}.')
