...
// box contains something that will solve all of Santa's
// transportation problems - at least, that's what it looks like from
// the pictures in the instructions." It doesn't seem like they can
// read whatever language it's in, but you can: "Sleigh kit. Some
// assembly required."
...
let graph = input
  .split(separator: "\n")
  .reduce(into: [String: Set<String>]()) { graph, line in
      let words = line.split(separator: " ").map { String($0) }
      let (source, destination) = (words[1], words[7])
      if graph[source] == nil {
          graph[source] = Set<String>()
      }
      graph[destination, default: Set<String>()].insert(source)
  }

var dependencies = graph
var steps = ""
while true {
    let readyNode = dependencies
      .filter { _, nodeDependencies in nodeDependencies.isEmpty }
      .map { node, _ in node }
      .sorted()
      .first
    if let readyNode = readyNode {
        steps += readyNode
        dependencies.removeValue(forKey: readyNode)
        for node in dependencies.keys {
            dependencies[node]?.remove(readyNode)
        }
    } else {
        break
    }
...
// In this example, it would take 15 seconds for two workers to
// complete these steps.

// With 5 workers and the 60+ second step durations described above,
// how long will it take to complete all of the steps?

func duration(of step: String) -> Int {
    let stepValue = Character(step).unicodeScalars.first?.value ?? 0
    let aValue = Unicode.Scalar("A").value 
    return Int(61 + stepValue - aValue)
}


var stepsWaiting = [String](graph.keys)
var stepsWorking = [String]()
var stepsDone = [String]()
var stepDurations = [String: Int](uniqueKeysWithValues: graph.keys.map { ($0, duration(of: $0)) })
var second = 0
let maximumStepsWorking = 5

while !stepsWaiting.isEmpty || !stepsWorking.isEmpty {
    let stepsReady = stepsWaiting
      .filter { step in
          let dependenciesReady = graph[step]?.allSatisfy { dependency in
              stepsDone.contains(dependency)
          }
          return dependenciesReady ?? true
      }
      .sorted()
      .prefix(maximumStepsWorking - stepsWorking.count)
    if !stepsReady.isEmpty {
        stepsWaiting = stepsWaiting.filter { stepReady in
            !stepsReady.contains(stepReady)
        }
        stepsWorking.append(contentsOf: stepsReady)
    }

    second += 1

    stepDurations = [String: Int](uniqueKeysWithValues: stepDurations.map { step, duration in
        if stepsWorking.contains(step) {
    ...
    stepsDone.append(contentsOf: stepsWorking.filter { step in
        stepDurations[step] == 0
    })
    stepsWorking = stepsWorking.filter { step in
        stepDurations[step] != 0
    }
}

print("Completed at second \(second)")
