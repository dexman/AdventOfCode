...
struct Example {
    let before: [Int]
    let instruction: (Int, OpcodeArgs)
    let after: [Int]
}

func parseExamples(from input: String) -> [Example] {
    func parseRegisters(from string: String) -> [Int] {
        return string
    ...
    var current: (before: [Int]?, instruction: (Int, OpcodeArgs)?) = (nil, nil)
    var examples = [Example]()
    for line in input.split(separator: "\n") {
        if line.starts(with: "Before: ") {
            current.before = parseRegisters(
              from: line.replacingOccurrences(of: "Before: ", with: ""))
        } else if line.starts(with: "After: ") {
            let after = parseRegisters(
              from: line.replacingOccurrences(of: "After: ", with: ""))
        ...
        } else if current.before != nil, !line.isEmpty {
            let instruction = line
              .split(separator: " ")
              .map { Int($0)! }
            current.instruction = (instruction[0], (instruction[1], instruction[2], instruction[3]))
        }
    }
    return examples
}
...
let examples = parseExamples(from: input)

let opcodesByName: [String: (CPU) -> (OpcodeArgs) -> ()] = [
  "addr": CPU.addr,
  "addi": CPU.addi,
  "mulr": CPU.mulr,
  "muli": CPU.muli,
...
let ambiguousExamples = examples
  .filter { example in
      let possibleOpcodes = opcodesByName.values.filter { opcode in
          let cpu = CPU()
          cpu.registers = example.before
          opcode(cpu)(example.instruction.1)
          return cpu.registers == example.after
      }
      return possibleOpcodes.count > 2
  }
print("Part 1 ambiguous examples count is: \(ambiguousExamples.count) of \(examples.count) total.")
...
func parseProgram(from input: String) -> [(Int, OpcodeArgs)] {
    guard let separator = input.range(of: "\n\n\n\n") else {
        return []
    }
    return input[separator.upperBound...]
      .split(separator: "\n")
      .map { line in
          let instruction = line
            .split(separator: " ")
...
func resolveOpcodeNumbers() -> [Int: String] {
    var opcodesByNumber = [Int: String]()
    while opcodesByNumber.count < opcodesByName.count {
        let results = examples
          .map { example -> (Example, [String])  in
              let possibleOpcodes = opcodesByName.filter { name, opcode in
                  if opcodesByNumber.values.contains(name) {
                      return false
                  }
                  let cpu = CPU()
                  cpu.registers = example.before
              ...
              return (example, Array(possibleOpcodes.keys))
          }
          .filter {
              $0.1.count == 1
          }
        for (example, names) in results {
            guard let name = names.first else { continue }
            opcodesByNumber[example.instruction.0] = name
        }
    }
    return opcodesByNumber
}

let opcodesByNumber = resolveOpcodeNumbers()
let cpu = CPU()
for (opcode, args) in parseProgram(from: input) {
    let opcodeFunction = opcodesByName[opcodesByNumber[opcode]!]!
    opcodeFunction(cpu)(args)
}
print("Part 2 registers at end: \(cpu.registers)")
