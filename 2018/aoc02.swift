...
func countLetters<T>(in boxId: T) -> [Character: Int] where T: StringProtocol {
...
func checksum<T>(_ boxIds: [T]) -> Int where T: StringProtocol {
    typealias ChecksumData = (contains2: Int, contains3: Int)
    let checksumData: ChecksumData = boxIds
      .map(countLetters)
      .reduce((contains2: 0, contains3: 0)) { (total: ChecksumData, boxIdLetterCounts: [Character: Int]) -> ChecksumData in
          (
            contains2: total.contains2 + (boxIdLetterCounts.values.contains(2) ? 1 : 0),
            contains3: total.contains3 + (boxIdLetterCounts.values.contains(3) ? 1 : 0)
          )
      }
    return checksumData.contains2 * checksumData.contains3
}
...
print("Part 1 checksum: \(checksum(boxIds))")
...
func arePrototypeBoxIds(_ lhsBoxId: String, _ rhsBoxId: String) -> Bool {
    let differenceCount = zip(lhsBoxId, rhsBoxId).filter(!=).count
    return differenceCount == 1 && lhsBoxId.count == rhsBoxId.count
}

var commonLetters: String = ""
for lhsBoxId in boxIds {
    for rhsBoxId in boxIds {
        if lhsBoxId != rhsBoxId, arePrototypeBoxIds(lhsBoxId, rhsBoxId) {
            commonLetters = String(zip(lhsBoxId, rhsBoxId).filter(==).map { $0.0 })
            break
        }
    }
}
...
print("Part 2 common letters: \(commonLetters)")
