...
// hasn't come back in a few hours, and he's not sure where he
// is. Scanning the region briefly, you discover one life signal in a
// cave system nearby; his friend must have taken shelter there. The
// man asks if you can go there to retrieve his friend.
...
// the depth of the cave system and the coordinates of the
// target. However, it does not reveal the type of each region. The
// mouth of the cave is at 0,0.
...
// For example, suppose the cave system's depth is 510 and the
// target's coordinates are 10,10. Using % to represent the modulo
// operator, the cavern would look as follows:
...
// Before you go in, you should determine the risk level of the
// area. For the the rectangle that has a top-left corner of region
// 0,0 and a bottom-right corner of the region containing the target,
// add up the risk level of each individual region: 0 for rocky
// regions, 1 for wet regions, and 2 for narrow regions.

// In the cave system above, because the mouth is at 0,0 and the
// target is at 10,10, adding up the risk level of all regions with an
// X coordinate from 0 to 10 and a Y coordinate from 0 to 10, this
// total is 114.
...
let depth = 10914
let target = Position(x: 9, y: 739)
let mouth = Position(x: 0, y: 0)

var geologicIndexCache = [Position: Int]()
func geologicIndex(of position: Position) -> Int {
    if let result = geologicIndexCache[position] {
        return result
    }
    ...
    let result: Int = {
        if position == mouth {
            ...
            let lhs = erosionLevel(
              of: Position(x: position.x - 1, y: position.y))
            let rhs = erosionLevel(
              of: Position(x: position.x, y: position.y - 1))
            return lhs * rhs
        }
    }()
    geologicIndexCache[position] = result
    return result
}

func erosionLevel(of position: Position) -> Int {
    return (geologicIndex(of: position) + depth) % 20183
}

func regionType(of position: Position) -> RegionType {
    let level = erosionLevel(of: position) % 3
    if level == 0 {
        return .rocky
    } else if level == 1 {
        return .wet
...
func riskLevel(of position: Position) -> Int {
    var result = 0
    for y in 0...position.y {
        for x in 0...position.x {
            let region = Position(x: x, y: y)
            switch regionType(of: region) {
            case .rocky:
                result += 0
            case .wet:
                result += 1
...
func format(size: (width: Int, height: Int)) -> String {
    return (0..<size.height).map { y in
...
print("Part 1 risk level: \(riskLevel(of: target))")
...
// traversed. The fastest route might involve entering regions beyond
...
// your new equipment would be valid for your current
// region. Switching to using the climbing gear, torch, or neither
// always takes seven minutes, regardless of which tools you start
// with. (For example, if you are in a rocky region, you can switch
// from the torch to the climbing gear, but you cannot switch to
// neither.)
...
struct Node: Hashable, CustomStringConvertible {
    ...
    init(position: Position, tool: Tool) {
    ...
    var neighbors: [Node] {
        ...
        return positions
          ...
          .flatMap { position in
              regionType(of: position).usableTools.map { tool in
                  Node(position: position, tool: tool)
              }
          }
    }
}

func shortestPath(from source: Node, to target: Node) -> (Int, [Node])? {
    func cost(from source: Node, to target: Node) -> Int {
        var estimate = 0
        estimate += abs(target.position.y - source.position.y)
        estimate += abs(target.position.x - source.position.x)
        estimate += source.tool == target.tool ? 0 : 7
        return estimate
    }

    func reconstructPath(cameFrom: [Node: Node], current: Node) -> (Int, [Node]) {
        var totalCost = 0
        var totalPath = [current]
        var current = current
        while let next = cameFrom[current] {
            totalCost += cost(from: current, to: next)
            totalPath.append(next)
            current = next
        }
        return (totalCost, totalPath.reversed())
    }
        
    var visited = Set<Node>()
    var queue = Set<Node>([source])
    var cameFrom = [Node: Node]()
    var gScore: [Node: Int] = [source: 0]
    var fScore: [Node: Int] = [source: cost(from: source, to: target)]

    while true {
        guard
          let current = queue.min(by: { lhs, rhs in
...
let source = Node(position: mouth, tool: .torch)
let goal = Node(position: target, tool: .torch)
if let (cost, path) = shortestPath(from: source, to: goal) {
    print("Part 2 cost is \(cost). Path: \(path)")
} else {
    print("Part 2 failed: no path found.")
}
