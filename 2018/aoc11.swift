...
// power. The square must be entirely within the 300x300
// grid. Identify this square using the X,Y coordinate of its top-left
...
func fuelCellPowerLevel(at index: (x: Int, y: Int), gridSerial: Int) -> Int {
    let coordinate = (x: index.x + 1, y: index.y + 1)
...
func summedAreaTable(of table: [[Int]]) -> [[Int]] {
    var summedAreaTable: [[Int]] = table.map { row in
        [Int](repeating: 0, count: row.count)
    }
    for y in table.indices {
        for x in table[y].indices {
            summedAreaTable[y][x] = table[y][x]
            summedAreaTable[y][x] += y > 0 ? summedAreaTable[y - 1][x] : 0
            summedAreaTable[y][x] += x > 0 ? summedAreaTable[y][x - 1] : 0
            summedAreaTable[y][x] -= y > 0 && x > 0 ? summedAreaTable[y - 1][x - 1] : 0
        }
    }
    return summedAreaTable
}

func querySummedArea(at index: (x: Int, y: Int), in table: [[Int]], size: (width: Int, height: Int)) -> Int {
    let (x, y) = (index.x, index.y)
    let yMax = y + size.height - 1
    let xMax = x + size.width - 1

    assert(yMax < table.count)
    assert(xMax < table[yMax].count)

    var sum = table[yMax][xMax]
...
func largestSquarePowerLevel(in fuelCells: [[Int]], size: Int) -> (powerLevel: Int, index: (x: Int, y: Int))? {
    let sat = summedAreaTable(of: fuelCells)

    let indices: [(x: Int, y: Int)] = (0..<fuelCells.count - size).flatMap { y in
        (0..<fuelCells[y].count - size).map { x in
            (x: x, y: y)
        }
    }

    let powerLevels: [(powerLevel: Int, index: (x: Int, y: Int))] = indices.map { index in
        let level = querySummedArea(at: index, in: sat, size: (width: size, height: size))
        return (level, index)
    }

    return powerLevels.max { lhs, rhs in
        lhs.powerLevel < rhs.powerLevel
    }
}

let gridSerial = 3613
let fuelCellIndexRange = 0..<300
let fuelCells: [[Int]] = fuelCellIndexRange.map { y in
    fuelCellIndexRange.map { x in
        return fuelCellPowerLevel(at: (x: x, y: y), gridSerial: gridSerial)
    }
}

if let (_, index) = largestSquarePowerLevel(in: fuelCells, size: 3) {
    let coordinate = (x: index.x + 1, y: index.y + 1)
    print("Part 1 3x3 power level at \(coordinate).")
} else {
    print("Part 1 no fuel cells found.")
}
...
// What is the X,Y,size identifier of the square with the largest
// total power?

// Your puzzle input is still 3613.

let largestPowerLevel = (1...fuelCells.count)
  .compactMap { size -> (size: Int, powerLevel: Int, index: (x: Int, y: Int))? in
      return largestSquarePowerLevel(in: fuelCells, size: size).map {
          (size: size, powerLevel: $0.powerLevel, index: $0.index)
      }
  }
  .max { lhs, rhs in
      lhs.powerLevel < rhs.powerLevel
...
if let (size, _, index) = largestPowerLevel {
    let coordinate = (x: index.x + 1, y: index.y + 1)
    print("Part 2 \(size)x\(size) power level at \(coordinate).")
} else {
    print("Part 2 no fuel cells found.")
}
