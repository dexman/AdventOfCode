...
let input: String
do {
    input = try String(contentsOfFile: "aoc01.txt", encoding: .utf8)
} catch {
    print("Could not open aoc01.txt")
    exit(1)
}

let frequencyChanges = input
  .split(separator: "\n")
  .map { (line: String.SubSequence) -> Int in
      guard let value = Int(line) else {
          print("Could not parse frequency change: `\(line)`")
          exit(1)
      }
      return value
  }

let finalFrequency = frequencyChanges.reduce(0, +)
print("Part 1 resulting frequency: \(finalFrequency)")
...
var frequenciesSeen = Set<Int>([currentFrequency])
var done = false
while !done {
    for frequencyChange in frequencyChanges {
        currentFrequency += frequencyChange
        if !frequenciesSeen.insert(currentFrequency).inserted {
            done = true
            break
        }
    }
...
print("Part 2 frequency seen twice: \(currentFrequency)")
